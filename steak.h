...
public:
    ...
    //Инкапсуляция - парадигма ООП, которая представляет собой способность скрывать определённые участки кода,
    //забирая тем самым возможность у субъекта нарушать целостность данного кода, вмешиваться в реализацию и тд
    /*Инкапсуляция обеспечивает своего рода целостность объекта, а сокрытие - скрывает детали о процессе.
     * Так как данный метод является константным, он не нарушает принцип инкапсуляции, но нарушает сокрытие.
     * Метод необходим для удобства дальнейшего использования стека, конкретно в методе writePerson класса PersonKeeper
    */
